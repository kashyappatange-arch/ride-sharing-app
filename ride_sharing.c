...
/*
    ...
    - A min-heap is used at request time to pick the nearest available driver (demonstrates heap)
    ...
    Additions:
    - ride tracking (Ride structs) so requests produce a rideId which can be ended via endRide(rideId)
    - findShortestPath(start,end) - a simple educational Manhattan path printer
*/
...
typedef struct Ride {
    int id;
    int driverIndex; /* index into drivers[] */
    int userId;
    Point pickup;
    Point dest;
    int active; /* 1 = ongoing, 0 = finished */
} Ride;

static Ride *rides = NULL;
static int rides_count = 0;
static int rides_capacity = 0;
static int next_ride_id = 1;

static void ensure_rides_capacity(void) {
    if (rides_count >= rides_capacity) {
        rides_capacity = rides_capacity ? rides_capacity * 2 : 8;
        rides = realloc(rides, rides_capacity * sizeof(Ride));
        if (!rides) { perror("realloc"); exit(1); }
    }
}
...
int requestRide(int userId, Point pickup, Point destination) {
    ...
    ensure_rides_capacity();
    rides[rides_count].id = next_ride_id;
    rides[rides_count].driverIndex = didx;
    rides[rides_count].userId = userId;
    rides[rides_count].pickup = pickup;
    rides[rides_count].dest = destination;
    rides[rides_count].active = 1;
    int rideId = next_ride_id++;
    rides_count++;

    printf("Assigned driver %d to user %d (distance %.2f). Ride id: %d\n", driverId, userId, best.dist, rideId);
    ...
    /* driver's location will be updated when the ride ends via endRide(rideId) */
...
void endRide(int rideId) {
    for (int i = 0; i < rides_count; ++i) {
        if (rides[i].id == rideId) {
            if (!rides[i].active) { printf("Ride %d is already finished.\n", rideId); return; }
            rides[i].active = 0;
            int didx = rides[i].driverIndex;
            drivers[didx].available = 1;
            drivers[didx].loc = rides[i].dest;
            printf("Ride %d ended. Driver %d is now available at (%d,%d).\n", rideId, drivers[didx].id, drivers[didx].loc.x, drivers[didx].loc.y);
            /* after freeing driver, try to serve pending requests */
            try_process_pending();
            return;
        }
    }
    printf("No active ride found with id %d.\n", rideId);
}

void findShortestPath(Point start, Point end) {
    printf("Shortest path from (%d,%d) to (%d,%d):\n", start.x, start.y, end.x, end.y);
    /* very simple Manhattan path (educational) */
    Point cur = start;
    while (cur.x != end.x) {
        if (end.x > cur.x) cur.x++; else cur.x--;
        printf("  (%d,%d)\n", cur.x, cur.y);
    }
    while (cur.y != end.y) {
        if (end.y > cur.y) cur.y++; else cur.y--;
        printf("  (%d,%d)\n", cur.x, cur.y);
    }
}
...
static void try_process_pending(void) {
    ...
    for (int attempt = 0; attempt < PENDING_CAP && pending_size > 0; ++attempt) {
        ...
        /* match: mark driver busy and create ride record */
        drivers[best_idx].available = 0;
        ensure_rides_capacity();
        rides[rides_count].id = next_ride_id;
        rides[rides_count].driverIndex = best_idx;
        rides[rides_count].userId = r.userId;
        rides[rides_count].pickup = r.pickup;
        rides[rides_count].dest = r.dest;
        rides[rides_count].active = 1;
        int createdId = next_ride_id++;
        rides_count++;
        printf("(From queue) Assigned driver %d to user %d (distance %.2f). Ride id: %d\n", drivers[best_idx].id, r.userId, best_dist, createdId);
        /* pop queue */
        Request popped;
        queue_pop(&popped);
        served++;
