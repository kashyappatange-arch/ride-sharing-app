...
New/Updated API
---------------
- `requestRide(userId, pickup, destination)` now returns a unique `rideId` (>0) when a driver is assigned, or `-1` if the request is queued.
- `endRide(rideId)` ends an active ride, marks the driver available, and updates the driver's location to the ride destination.
- `findShortestPath(start, end)` prints a simple Manhattan-style shortest path between two points (educational helper).

Coding practices and structure
-----------------------------
The project now follows a clearer separation:

a) Header file: `ride_sharing.h` — contains public function declarations and the `Point` type.

b) Source file: `ride_sharing.c` — contains implementations of the API and internal DSA structures.

c) Main file: `main.c` — contains the interactive CLI and `main()`.

Notes on coding practices used in the project:
- Dynamic memory allocation is used for driver and ride arrays; helper functions ensure capacity and check return values.
- Internal state is kept in `static` variables inside `ride_sharing.c`. For a larger project you may prefer to encapsulate state in a context struct and avoid file-scope globals.
- Boundary conditions (empty queues, missing driver IDs, duplicate requests) are checked and reported.
- Functions return status codes where appropriate (e.g., `requestRide` returns `-1` when queued or on failure).
- Basic resource handling is implemented; for production code run tools like Valgrind to check for leaks.
- Naming follows a simple, consistent snake_case-like style for functions and lowercase for variables.
